...
class submit implements ActionListener, ItemListener
{
	...
	{			
		...
		String query = "SELECT * FROM PAT WHERE patientno='" + num+ "'";
 		ResultSet rs = st.executeQuery(query);
                int foundrec = 0;
                while (rs.next())
                {
                   
                   JOptionPane.showMessageDialog(bsub, "Patient id Already Exists in DataBase!!!");
		...
		new SuccessDialog();
                ...
                else
     		{
                    ...
                    JOptionPane.showMessageDialog(bsub, "Empty Record !!!");
